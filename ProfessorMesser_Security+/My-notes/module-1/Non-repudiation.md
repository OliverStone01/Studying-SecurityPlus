...
Similar methods can be seen in cryptography.
...
If you take the same data that was previously hashed, even if the data is a whole book and you only change 1 letter on a random page, the hash value will be completely different from the original hash value.
...
Alice then sends the plaintext message and the digital signature to Bob via email or some type of digital delivery. Bob will then receive the plaintext message along with the digital signature.

For Bob to make sure that the message has not been modified in anyway and that it came from Alice, he must decrypt the digital signature using Alice's public key then hash the plaintext and compare the values.
